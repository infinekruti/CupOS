...
void setup()
{
    ...
    // Enable Watchdog Timer (45 seconds timeout to cover GSM latency)
    ...
    if (TEST_MODE) {
        if (Serial.available()) {
            ...
            switch (cmd) {
                ...
                case 'q': {
                    uint8_t prodId; uint16_t dur; String pName; bool isHalf;
                    diagnostics.info(ModuleID::System, "Simulating network request...");
                    net.verifyOrder("mock_qr_token_123", prodId, dur, pName, isHalf);
                    break;
                }
                case 'g': {
