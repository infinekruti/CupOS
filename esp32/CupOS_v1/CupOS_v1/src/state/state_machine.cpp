...
bool StateMachine::processQR() {
    if (_qr && _qr->available()) {
        ...
        bool isHalf;
        String failReason = "";
        if (_net && _net->verifyOrder(payload, prodId, dur, pName, isHalf, failReason)) {
            _productId = prodId;
            _dispenseMs = dur;
            _productName = pName;
            _isHalf = isHalf;
            ...
            displayManager.showMessage(failReason.c_str());
            // WDT-safe 3-second delay so user can read the error
            uint32_t errStart = millis();
            while (millis() - errStart < 3000) {
                esp_task_wdt_reset();
                delay(10);
            }
            // Return to QR idle screen
...
void StateMachine::update() {
    ...
    switch (_state) {
        ...
        case CupOSState::BeveragePrep:
            ...
            Serial.println(">>> [SM] About to play CupOS.wav <<<");
            delay(50); // Let SPI bus settle after display writes
